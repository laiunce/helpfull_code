...
@author: DAB97664
"""
#Este algorito calcula el lift min, lift max, max |LIFT-1| y cantidad de categorias (incluyendo el NULL), de todas las categorias de cada variable, con respecto 
...
nombre_archivo_entrada = 'SET_ENTRENAMIENTO_201608_201610_3_20porc_B.csv'
nombre_archivo_salida = 'ANALISIS_VAR_SET_ENTRENAMIENTO_201608_201610_3_20porc_B.csv'
...
print('Hora Inicio: '+ str(hora_ini) )
...
# carga de datos
datos_categoricos=pd.read_csv(nombre_archivo_entrada,sep=delimitador_archivo,encoding='latin-1',low_memory=False)
# imputo nulos
datos_categoricos = datos_categoricos.fillna(valor_null) # completo los null con la palabra el númer -99999
...
analisis_lift_variables =pd.DataFrame( columns= ['variable', 'dif_lift_abs', 'max_lift','min_lift','Cant_Categorias', 'Flag_tiene_NULL' ] )


for var in nom_columnas:
    #list(set(datos_categoricos[target])) # distinct target
    #list(set(datos_categoricos[var])) # distinct variable
    cant_categ = len(list(set(datos_categoricos[var])))  # calcula la cantidad de categorias de la variable
    if valor_null in list(set(datos_categoricos[var])): # si la variable tiene nulls, marca en un flag
        flag_tiene_null = 1
    else:
        flag_tiene_null = 0
    
    if ( cant_categ < min_cant_categorias == 50):
        ...
        analisis_lift_variables = analisis_lift_variables.append(pd.DataFrame( [[var, dif_lift_abs , max_lift, min_lift,cant_categ,flag_tiene_null] ], columns= ['variable', 'dif_lift_abs', 'max_lift','min_lift','Cant_Categorias', 'Flag_tiene_NULL' ] ))               
        ...
        analisis_lift_variables = analisis_lift_variables.append(pd.DataFrame( [[var, None , None, None,cant_categ,flag_tiene_null] ], columns= ['variable', 'dif_lift_abs', 'max_lift','min_lift' ,'Cant_Categorias', 'Flag_tiene_NULL'] ))               
    
analisis_lift_variables.to_csv(nombre_archivo_salida, sep='\t')

hora_fin = datetime.datetime.now()
duracion_corrida = hora_fin-hora_ini
print('Hora Finalizacion: '+ str(hora_fin) )
print('Duracion: ' + str(duracion_corrida) )
