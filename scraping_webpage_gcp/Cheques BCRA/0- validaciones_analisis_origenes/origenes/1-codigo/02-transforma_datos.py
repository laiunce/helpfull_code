...
for i in tmp:
    ...
    #Harcodeado por unica razon VER BIEN ESTO SI CAMBIA!!!!!
    causal = monto_fecha_pago
    causal['causal'] = 'Sin Fondo'
    causal=causal.drop('monto_fechapago', axis=1)
    causal=causal.drop('fechapago', axis=1)



    monto_cheque = monto_fecha_pago.apply(lambda row: formatea_monto_dec(row['monto_fechapago'],row['fechapago']), axis=1)

    data_al_dataframe = pd.concat([columna_1,nro_cheques_df,fechas_rechazo_df,monto_cheque,fechas_pago_df,causal,columna_5,columna_4], axis=1)
    data_al_dataframe.columns = ['cuil','nro_cheque','fecha_rechazo','monto','fecha_pago','causal','pago_multa','persona_juridica_relacionada']
